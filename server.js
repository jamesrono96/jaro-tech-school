...
const assessmentTypes = [
  {id:"at1",name:"CAT 1",max_score:100,weight:20,active:true},
  {id:"at2",name:"Mid Term",max_score:100,weight:30,active:true},
  {id:"at3",name:"End Term",max_score:100,weight:50,active:true}
];

// Each subject may have its own grading scale, or inherit another subject's scale.
// The administrator controls these rules.
const subjectGradings = new Map();
const subjects = [
...
for (const sub of subjects) subjectGradings.set(sub.id,{mode:"custom",source_subject_id:null,rules:gradingRules.map(r=>({...r,id:`${sub.id}-${r.id}`}))});
...
for(let s=0;s<5;s++) for(let j=0;j<3;j++){
  const type=assessmentTypes[j];
  assessments.push({
    id:`a${s*3+j}`,type_id:type.id,name:type.name,term:"Term 2",
    max_score:type.max_score,weight:type.weight,subject_id:subjects[s].id,status:"open",submitted:false,submitted_by:null,submitted_at:null
  });
}
...
function resolveSubjectRules(subjectId, seen=new Set()){
  const cfg=subjectGradings.get(subjectId);
  if(!cfg) return gradingRules;
  if(cfg.mode==="same" && cfg.source_subject_id && !seen.has(subjectId)){
    seen.add(subjectId);
    return resolveSubjectRules(cfg.source_subject_id, seen);
  }
  return Array.isArray(cfg.rules)&&cfg.rules.length ? cfg.rules : gradingRules;
}
function gradingFor(subjectId,p){
  const n=Number(p)||0;
  const rules=resolveSubjectRules(subjectId);
  return rules.find(r=>n>=Number(r.min)&&n<=Number(r.max)) || rules[rules.length-1];
}
function grade(subjectId,p){return gradingFor(subjectId,p).grade;}
function remark(subjectId,g){return resolveSubjectRules(subjectId).find(r=>r.grade===g)?.remark || "";}
...
app.get("/health",(req,res)=>res.json({ok:true,service:"Jaro-Tech School Management System",version:"10.0"}));
...
app.get("/api/admin/assessment-types",auth,roles("admin","hod","exam_officer"),(req,res)=>res.json({types:assessmentTypes}));
app.post("/api/admin/assessment-types",auth,roles("admin","hod"),(req,res)=>{
  const name=String(req.body.name||"").trim();
  const maxScore=Number(req.body.max_score||100), weight=Number(req.body.weight||0);
  if(!name)return res.status(400).json({error:"assessment_type_name_required"});
  if(assessmentTypes.some(x=>x.name.toLowerCase()===name.toLowerCase()))return res.status(409).json({error:"assessment_type_exists"});
  const type={id:crypto.randomUUID(),name,max_score:maxScore>0?maxScore:100,weight:Number.isFinite(weight)?weight:0,active:req.body.active!==false};
  assessmentTypes.push(type);res.status(201).json({ok:true,type});
});
app.patch("/api/admin/assessment-types/:id",auth,roles("admin","hod"),(req,res)=>{
  const t=assessmentTypes.find(x=>x.id===req.params.id);if(!t)return res.status(404).json({error:"assessment_type_not_found"});
  if(req.body.name!==undefined)t.name=String(req.body.name).trim();
  if(req.body.max_score!==undefined)t.max_score=Math.max(1,Number(req.body.max_score)||100);
  if(req.body.weight!==undefined)t.weight=Math.max(0,Number(req.body.weight)||0);
...
app.post("/api/admin/assessments",auth,roles("admin","hod","exam_officer"),(req,res)=>{
  const b=req.body||{}, sub=subjects.find(s=>s.id===b.subject_id), type=assessmentTypes.find(t=>t.id===b.type_id && t.active);
  if(!sub||!type)return res.status(400).json({error:"subject_and_assessment_type_required"});
  const a={id:crypto.randomUUID(),type_id:type.id,name:type.name,term:b.term||school.current_term,max_score:Number(b.max_score)||type.max_score,weight:type.weight,subject_id:sub.id,status:"open",submitted:false,submitted_by:null,submitted_at:null};
...
app.get("/api/results/overview",auth,roles("admin","hod","exam_officer"),(req,res)=>{
  ...
  const rows=list.map(s=>({...s,academic_level:s.class_name,average:studentAverage(s.id),grade:grade(null,studentAverage(s.id)),entries:[...marks.values()].filter(m=>m.student_id===s.id).length}));
...
app.get("/api/admin/grading",auth,roles("admin","hod"),(req,res)=>{
  const subjectId=req.query.subject_id;
  if(subjectId){
    const cfg=subjectGradings.get(subjectId)||{mode:"custom",source_subject_id:null,rules:gradingRules};
    return res.json({subject_id:subjectId,mode:cfg.mode,source_subject_id:cfg.source_subject_id,rules:resolveSubjectRules(subjectId)});
  }
  res.json({rules:gradingRules,subjects:subjects.map(s=>{const c=subjectGradings.get(s.id);return {id:s.id,name:s.name,mode:c?.mode||"custom",source_subject_id:c?.source_subject_id||null,rules:resolveSubjectRules(s.id)};})});
});
app.patch("/api/admin/grading",auth,roles("admin","hod"),(req,res)=>{
  const subjectId=req.body.subject_id;
  if(!subjects.some(s=>s.id===subjectId))return res.status(400).json({error:"subject_required"});
  if(req.body.mode==="same"){
    const source=req.body.source_subject_id;
    if(!subjects.some(s=>s.id===source)||source===subjectId)return res.status(400).json({error:"valid_source_subject_required"});
    subjectGradings.set(subjectId,{mode:"same",source_subject_id:source,rules:[]});
    return res.json({ok:true,subject_id:subjectId,mode:"same",source_subject_id:source,rules:resolveSubjectRules(subjectId)});
  }
  ...
  subjectGradings.set(subjectId,{mode:"custom",source_subject_id:null,rules:cleaned});
  res.json({ok:true,subject_id:subjectId,mode:"custom",rules:cleaned});
});
...
app.get("/api/results/student",auth,(req,res)=>{
  ...
  const rs=[...marks.values()].filter(m=>m.student_id===s.id&&(req.user.role!=="parent"||m.approved)).map(m=>{
    const a=assessments.find(x=>x.id===m.assessment_id),sub=subjects.find(x=>x.id===a.subject_id),p=m.score/a.max_score*100,g=grade(sub.id,p);
    return {subject_id:sub.id,subject_name:sub.name,assessment_id:a.id,assessment_name:a.name,score:m.score,max_score:a.max_score,percentage:p,grade:g,remark:remark(sub.id,g),approved:m.approved,locked:m.locked};
  });
...
app.get("/api/results/approval",auth,roles("admin","hod","exam_officer"),(req,res)=>{
  ...
  res.json({submitted:rows.filter(m=>m.submitted).length,approved:rows.filter(m=>m.approved).length,locked:rows.filter(m=>m.locked).length,rows,assessment:assessments.find(a=>a.id===req.query.assessment_id)});
});
app.post("/api/results/submit",auth,roles("teacher"),(req,res)=>{
  const a=assessments.find(x=>x.id===req.body.assessment_id); if(!a)return res.status(404).json({error:"assessment_not_found"});
  if(a.status==="locked")return res.status(400).json({error:"assessment_locked",message:"This assessment is already locked."});
  const t=teachers.find(x=>x.id===req.user.teacher_id);
  if(!t || !teacherSubjects(t).includes(a.subject_id))return res.status(403).json({error:"wrong_subject"});
  const assigned=students.filter(st=>teacherHasAssignment(t,a.subject_id,st.class_name,st.stream));
  const ms=assigned.map(st=>marks.get(`${a.id}:${st.id}`)).filter(Boolean);
  if(ms.length<assigned.length)return res.status(400).json({error:"marks_incomplete",message:"All students must have marks before submission."});
  if(ms.some(m=>m.score===null||m.score===undefined))return res.status(400).json({error:"marks_incomplete"});
  a.submitted=true; a.submitted_by=req.user.email; a.submitted_at=new Date().toISOString();
  for(const m of ms){m.submitted=true;m.submitted_by=req.user.email;m.submitted_at=a.submitted_at;}
  res.json({ok:true,message:"Marks confirmed and submitted to administration for review."});
});
app.post("/api/results/approve",auth,roles("admin","hod","exam_officer"),(req,res)=>{
  const a=assessments.find(x=>x.id===req.body.assessment_id);if(!a)return res.status(404).json({error:"assessment_not_found"});
  if(!a.submitted)return res.status(400).json({error:"teacher_confirmation_required",message:"The teacher must confirm and submit the marks before administration can approve them."});
...
app.post("/api/admin/unlock-requests/:id/decision",auth,roles("admin","hod","exam_officer"),(req,res)=>{
  ...
  if(decision==="approve"){const a=assessments.find(x=>x.id===r.assessment_id);if(a){a.status="open";a.submitted=false;a.submitted_by=null;a.submitted_at=null;for(const m of marks.values())if(m.assessment_id===a.id){m.locked=false;m.approved=false;m.submitted=false;}}}
  res.json({ok:true,request:r});
});
app.post("/api/admin/assessments/:id/unlock",auth,roles("admin","hod","exam_officer"),(req,res)=>{
  const a=assessments.find(x=>x.id===req.params.id);if(!a)return res.status(404).json({error:"assessment_not_found"});
  a.status="open";a.submitted=false;a.submitted_by=null;a.submitted_at=null;for(const m of marks.values())if(m.assessment_id===a.id){m.locked=false;m.approved=false;m.submitted=false;}res.json({ok:true,assessment:a});
});
...
app.get("/api/attendance/list",auth,(req,res)=>{
  ...
  if(req.user.role!=="teacher") {
    const submittedClasses=new Set([...attendanceSubmissions.keys()].filter(k=>k.startsWith(`${date}|${session}|`)).map(k=>k.split("|").slice(2).join("|")));
    list=list.filter(s=>submittedClasses.has(`${s.class_name}|${s.stream}`));
  }
  res.json(list.map(s=>({...s,status:attendance.get(`${s.id}:${date}:${session}`)?.status||"Present",note:attendance.get(`${s.id}:${date}:${session}`)?.note||"",session,submitted:!!attendanceSubmissions.get(`${date}|${session}|${s.class_name}|${s.stream}`)})));
});
app.post("/api/attendance/save",auth,roles("teacher"),(req,res)=>{
  const s=students.find(x=>x.id===req.body.student_id);if(!s)return res.status(404).json({error:"student_not_found"});
  if(!isClassTeacher(req.user.teacher_id,s.class_name,s.stream))return res.status(403).json({error:"not_class_teacher",message:"Only the assigned class teacher can record attendance for this class."});
  const date=req.body.attendance_date||new Date().toISOString().slice(0,10), session=req.body.session||"morning";
  if(!["morning","evening"].includes(session))return res.status(400).json({error:"invalid_session"});
  const key=`${date}|${session}|${s.class_name}|${s.stream}`;
  if(attendanceSubmissions.has(key))return res.status(409).json({error:"attendance_submitted",message:"Attendance has already been submitted to administration for this class and session."});
  attendance.set(`${s.id}:${date}:${session}`,{status:req.body.status||"Present",note:req.body.note||"",recorded_by:req.user.teacher_id});res.json({ok:true});
});
app.post("/api/attendance/submit",auth,roles("teacher"),(req,res)=>{
  const date=req.body.attendance_date||new Date().toISOString().slice(0,10), session=req.body.session||"morning", className=String(req.body.class_name||""), stream=String(req.body.stream||"");
  if(!["morning","evening"].includes(session)||!className||!stream)return res.status(400).json({error:"attendance_submission_details_required"});
  if(!isClassTeacher(req.user.teacher_id,className,stream))return res.status(403).json({error:"not_class_teacher"});
  const key=`${date}|${session}|${className}|${stream}`;
  if(attendanceSubmissions.has(key))return res.status(409).json({error:"already_submitted"});
  const ss=students.filter(s=>s.class_name===className&&s.stream===stream);
  if(!ss.length)return res.status(400).json({error:"no_students"});
  for(const st of ss) if(!attendance.has(`${st.id}:${date}:${session}`)) attendance.set(`${st.id}:${date}:${session}`,{status:"Present",note:"",recorded_by:req.user.teacher_id});
  attendanceSubmissions.set(key,{date,session,class_name:className,stream,teacher_id:req.user.teacher_id,submitted_at:new Date().toISOString()});
  res.json({ok:true,submission:attendanceSubmissions.get(key)});
});
app.get("/api/admin/attendance-overview",auth,roles("admin","hod"),(req,res)=>{
  const date=req.query.date||new Date().toISOString().slice(0,10), session=req.query.session||"morning";
  const submissions=[...attendanceSubmissions.values()].filter(x=>x.date===date&&x.session===session).map(x=>({...x,teacher_name:teachers.find(t=>t.id===x.teacher_id)?.full_name||""}));
  const submittedKeys=new Set(submissions.map(x=>`${x.class_name}|${x.stream}`));
  const rows=students.filter(s=>submittedKeys.has(`${s.class_name}|${s.stream}`)).map(s=>({...s,status:attendance.get(`${s.id}:${date}:${session}`)?.status||"Present",note:attendance.get(`${s.id}:${date}:${session}`)?.note||""}));
  const summary={total:rows.length,present:rows.filter(x=>x.status==="Present").length,absent:rows.filter(x=>x.status==="Absent").length,late:rows.filter(x=>x.status==="Late").length,excused:rows.filter(x=>x.status==="Excused").length};
  res.json({date,session,submissions,summary,rows});
});
